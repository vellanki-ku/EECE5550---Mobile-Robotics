...
# ============== HELPER FUNCTIONS ==============
def create_random_map(obstacle_count=OBSTACLE_COUNT):
    obstacles = []
...
def simulate_lidar(x, y, theta, obstacles, noise_std=LIDAR_NOISE_STD):
    ...
    for angle in angles:
        ...
        for r in range(0, LIDAR_RANGE, 2):
            ...
            if any(obs.collidepoint(rx, ry) for obs in obstacles):
                scans.append([rx + np.random.normal(0, noise_std), ry + np.random.normal(0, noise_std)])
                break
        else:
...
def run_single_simulation(obstacle_count=OBSTACLE_COUNT, lidar_noise_std=LIDAR_NOISE_STD, run_visualization=True):
    pygame.init()
    screen = None
    if run_visualization:
        screen = pygame.display.set_mode(MAP_SIZE)
        pygame.display.set_caption(f"CBGL Navigation (Obs: {obstacle_count}, Noise: {lidar_noise_std})")
    clock = pygame.time.Clock()

    obstacles, grid = create_random_map(obstacle_count)
    ...
    running = True
    while running:
        if run_visualization and screen is not None:
            screen.fill((255, 255, 255))
            draw_obstacles(screen, obstacles)
            draw_goal(screen)
            draw_path(screen, path_cells)
        ...
        noisy_scan = simulate_lidar(robot_pos[0], robot_pos[1], robot_theta, obstacles, lidar_noise_std)
    ...
    pygame.quit()
    return log, trajectory_gt, trajectory_est

def run_multiple_simulations(num_runs=5, obstacle_counts=[100], lidar_noise_stds=[2], visualize_one=False):
    all_logs = {}
    all_trajectories_gt = {}
    all_trajectories_est = {}

    for obs_count in obstacle_counts:
        for noise_std in lidar_noise_stds:
            key = (obs_count, noise_std)
            logs = []
            trajectories_gt = []
            trajectories_est = []
            start_time = time.time()
            for i in range(num_runs):
                print(f"Running simulation {i+1}/{num_runs} with Obstacles: {obs_count}, Noise: {noise_std}")
                log, traj_gt, traj_est = run_single_simulation(obs_count, noise_std, visualize_one and i == 0)
                logs.append(np.array(log))
                trajectories_gt.append(np.array(traj_gt))
                trajectories_est.append(np.array(traj_est))
            end_time = time.time()
            print(f"Finished {num_runs} simulations for Obstacles: {obs_count}, Noise: {noise_std} in {end_time - start_time:.2f} seconds.")
            all_logs[key] = logs
            all_trajectories_gt[key] = trajectories_gt
            all_trajectories_est[key] = trajectories_est
    return all_logs, all_trajectories_gt, all_trajectories_est

def analyze_results(all_logs):
    print("\n--- Analysis of Multiple Simulations ---")
    for (obs_count, noise_std), logs in all_logs.items():
        if not logs:
            print(f"No data for Obstacles: {obs_count}, Noise: {noise_std}")
            continue
        pose_errors = np.concatenate([log[:, 0] for log in logs])
        theta_errors = np.concatenate([log[:, 1] for log in logs])

        print(f"\nResults for Obstacles: {obs_count}, Noise: {noise_std}:")
        print(f"  Average Pose Error: {np.mean(pose_errors):.2f}")
        print(f"  Std Dev Pose Error: {np.std(pose_errors):.2f}")
        print(f"  Max Pose Error: {np.max(pose_errors):.2f}")
        print(f"  Average Theta Error: {np.mean(theta_errors):.2f}")
        print(f"  Std Dev Theta Error: {np.std(theta_errors):.2f}")
        print(f"  Max Theta Error: {np.max(theta_errors):.2f}")

def plot_results(all_trajectories_gt, all_trajectories_est):
    print("\n--- Plotting Results ---")
    num_conditions = len(all_trajectories_gt)
    rows = int(math.ceil(math.sqrt(num_conditions)))
    cols = int(math.ceil(num_conditions / rows))
    fig_traj, axes_traj = plt.subplots(rows, cols, figsize=(15, 10))
    axes_traj = np.ravel(axes_traj)

    for i, ((obs_count, noise_std), trajectories_gt) in enumerate(all_trajectories_gt.items()):
        axes_traj[i].set_title(f"Obs: {obs_count}, Noise: {noise_std}")
        for j, traj_gt in enumerate(trajectories_gt):
            axes_traj[i].plot(traj_gt[:, 0], traj_gt[:, 1], label=f'GT Run {j+1}', linewidth=1)
            if (obs_count, noise_std) in all_trajectories_est and len(all_trajectories_est[(obs_count, noise_std)]) > j:
                axes_traj[i].plot(all_trajectories_est[(obs_count, noise_std)][j][:, 0], all_trajectories_est[(obs_count, noise_std)][j][:, 1], label=f'Est Run {j+1}', linestyle='--')
        axes_traj[i].scatter(*GOAL_POS, color='orange', label='Goal')
        axes_traj[i].set_xlabel("X")
        axes_traj[i].set_ylabel("Y")
        axes_traj[i].axis("equal")
        axes_traj[i].grid(True)
        if i == 0:
            axes_traj[i].legend()
    for j in range(i + 1, rows * cols):
        fig_traj.delaxes(axes_traj[j])
    plt.tight_layout()
    plt.show()

if __name__ == "__main__":
    #run_single_simulation(obstacle_count=100, lidar_noise_std=2, run_visualization=True)
    num_simulations = 10
    obstacle_variations = [50, 100]
    noise_variations = [1, 3]
    all_results_log, all_results_gt, all_results_est = run_multiple_simulations(
        num_simulations, obstacle_variations, noise_variations, visualize_one=True
    )
    analyze_results(all_results_log)
    plot_results(all_results_gt, all_results_est)